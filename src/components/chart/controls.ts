    ...
    updateLegend(event: MouseEventParams) {
        for (let i = 0; i < this.chart.loadedIndicators.length; i++) {
            ...
            for (let j = 0; j < indicator.apis.length; j++) {
                ...
                // antoine add legend - zzz zbuy
                const plot = indicator.model.plots[j]
                const showLegend = indicator.options.toggleShowLegend || false
                ...
                const title = plot?.options?.title || '';

                if (data && (text.length || enhancedText.length)) {
                    text += '\u00a0|\u00a0'
                    enhancedText += '\u00a0|\u00a0'
                }
                ...
                if (data.value && data.color) {
                    enhancedText += `<span style="color: ${data.color}">${title}: ${formatFunction(data.value, precision)}</span>`
                } else if (data.value) {
            ...
            enhancedText.length && (this.legendElements[id].innerHTML = enhancedText)
            enhancedText.length === 0 && (this.legendElements[id].textContent = text)
        }
    }
